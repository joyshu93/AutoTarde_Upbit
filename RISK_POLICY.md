...
- after renewal, the service must re-read account-wide active/uncertain orders and then authoritative execution state under the held lease; `submitOrderFromDecision` performs that execution-state read directly as the final await on the successful path before synchronous checks and the sole send, without a helper-resolution, callback, report, database, timer, or promise seam. Either read failure is a paused fail-closed condition. The execution adapter carries its own required discriminant instead of accepting an independent path label. The tagged `LIVE` adapter requires persisted `executionMode=LIVE` and `liveExecutionGate=ENABLED` both initially and finally. The tagged `DRY_RUN` adapter accepts any non-fully-live initial tuple, including `DRY_RUN`/`ENABLED` and `LIVE`/`DISABLED`, but requires the exact initial mode-and-gate tuple to remain unchanged at the final check. Any drift records lease-block evidence where possible, requires pause, retains recovery evidence, and sends nothing.
- for a candidate order, the post-renewal check must first use a bounded candidate-only repository contract covering active rows plus potentially dispatched `FAILED` and `REJECTED` recovery rows, excluding only absence-confirmed `FAILED`; saturation is unsafe. It must then sequentially re-read the exact account-plus-order-ID persisted order, its first event, the persisted `READY` decision, deployment, exact candidate state, and binding before the final execution-state read. The order must still be the exact expected local `SUBMITTING` lifecycle with no exchange or failure evidence; strict plain own enumerable data-property projections must match every expected record, and only status may be projected back to `PERSISTED` for the reviewed aggregate validator. A competing order, missing row, changed authority, or material mismatch uses the dedicated candidate-intent fault pause, retains the lease and bound order, and sends nothing.
