...
- after renewal, the service must re-read account-wide active/uncertain orders and then authoritative execution state under the held lease; execution state is the final awaited authority read before the sole send, and either read failure is a paused fail-closed condition
