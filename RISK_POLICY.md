...
The PositionGuard public backtest command is research-only. It may call Upbit public candle endpoints and run the pure offline replay/report path, but it must not read or write the local execution database, poll or deliver Telegram, call Upbit private endpoints, create strategy decisions in persistence, create order intents, mutate order lifecycle records, or transmit live orders.
