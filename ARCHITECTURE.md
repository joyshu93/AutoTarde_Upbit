...
The verification result carries an in-memory, non-rendered file-identity/open verifier. Ownership persistence, application persistence, and LIVE read-only inspection compare that identity before and after their SQLite open and validate the same migration/account/instance/credential contract on the actual opened handle before mutable initialization. A verified path replaced between composition phases is rejected before migrations, bootstrap, or ownership writes. Native canonical resolution, not a broad tilde regex, distinguishes a real Windows short-name alias from a legitimate filename such as `runtime~2026.sqlite`.
