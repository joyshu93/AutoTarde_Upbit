...
15. Renew the lease and re-read account-wide active orders. For a candidate order, then re-read in order its exact persisted `SUBMITTING` row, first event, `READY` decision, deployment, exact candidate state, and binding; explicitly validate all lifecycle and immutable intent material, project only status back to `PERSISTED`, and rerun the reviewed aggregate validator. Execution state remains the final awaited authority read. The exact initial mode/gate tuple must be unchanged; the tagged live adapter additionally requires `LIVE`/`ENABLED`, while the tagged dry adapter accepts any unchanged non-fully-live tuple. Then call that adapter exactly once with no intervening await.
