...
- The broader `HTF_TREND_GATE`, `STRICT_PULLBACK`, `EARLY_THESIS_FAILURE`, `ADD_LIMITED`, `COOLDOWN_CONTROL`, and `COMBINED_CONSERVATIVE` research scenarios are deferred. They are not inputs to the current ADD loss-attribution or holdout-classification modules and require a later, separately designed read-only research evaluation.
