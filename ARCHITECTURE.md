...
The runtime is a same-host modular-monolith boundary: exactly one mutating `DRY_RUN` or `LIVE` process may own a canonical SQLite database plus exchange account. One fail-closed local-path canonicalizer runs before lock derivation and every mutable or read-only database open. It rejects UNC/network and device paths, Windows short names, symlink/junction/reparse aliases, and files with ambiguous hardlink names. The accepted canonical path and account produce one mode-invariant scope digest used by the Windows named-pipe process lock, the exact database path opened by both modes, and the persisted `scope_key` on every current-owner operation and audit event. The supported topology is one Windows host with local storage; network shares and multi-host operation are intentionally unsupported.

Ownership begins before mutable application construction. The owner receives a monotonic persisted generation, renews every 10 seconds with a 45-second TTL, and becomes permanently fenced on process-lock loss, generation mismatch, or expiry. Every local admission assertion samples the injected runtime clock and treats `now >= expires_at` as terminal even before the heartbeat timer observes it. Loss recording appends best-effort exact-generation `LOST` evidence before the ownership database closes and cannot overwrite or mutate a newer owner; normal shutdown appends `RELEASED` instead. Shutdown fences first, stops notification delivery, Telegram polling, scheduler timers, and heartbeat renewal, waits up to 30 seconds for all in-flight work, including a scheduler startup-block report, to settle or records explicit timeout failure, releases only the exact generation when still current and quiescent, closes both SQLite connections, then releases the process lock.

Normal cleanup fencing and terminal ownership loss remain distinct even if loss occurs after cleanup starts. A process-lock loss escalates a normal fence to exact-generation `LOST` and makes `RELEASED` unavailable. Worker quiescence failure in either long-running or scoped shutdown likewise records `LOST` and retains application persistence, ownership persistence, and the process lock until terminal process exit; the caller fails instead of advertising a clean handoff while an outbound operation may still settle.

The sole reachable `createOrder` call still begins synchronously inside the final combined persisted-authority callback. Adapter fulfillment and adapter rejection are both followed by a fresh current-generation assertion before any order/fill/rejection persistence, execution-state pause, notification, or account-lease release. If ownership changed or the assertion is unavailable, the exact ownership/read error escapes and the old owner leaves `SUBMITTING` plus its lease for current-owner recovery. Existing short-lived `account_execution_leases`, strategy, sizing, risk, reconciliation, and Telegram truth contracts are otherwise separate and unchanged.
...
Migration `0024_add_runtime_ownership.sql` is an offline deployment operation, never a runtime startup action. Its current-owner primary key and every audit index/event are bound by the canonical 64-hex `scope_key`. Deployment still requires an explicit stop, database-and-sidecar backup, approved migration procedure, read-only readiness verification, one owner start, and separate duplicate-start rejection exercise.
...
`src/app/live-database-identity.ts` is a fail-closed composition-root guard, not trading logic. `DRY_RUN` remains creation-and-migration capable only beneath a verified local canonical ancestor. Before a LIVE runtime is constructed, the guard requires an absolute existing regular uniquely named local SQLite file, validates the canonical migration ledger read-only (including the explicitly retired historical `0013` record), verifies the primary Upbit KRW spot account row, and compares the persisted database instance UUID and domain-separated Upbit access-key fingerprint. It never creates, migrates, bootstraps, repairs, or auto-binds a LIVE database.

The verification result carries an in-memory, non-rendered file-identity/open verifier. Ownership persistence, application persistence, and LIVE read-only inspection compare that identity before open, then derive and canonicalize the main-database location reported by the opened SQLite connection and compare its file identity before validating the same migration/account/instance/credential contract. A verified path replacement or a different actual opened handle is rejected before migrations, bootstrap, or ownership writes. Native canonical resolution, not a broad tilde regex, distinguishes a real Windows short-name alias from a legitimate filename such as `runtime~2026.sqlite`.
