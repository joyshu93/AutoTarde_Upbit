...
Candidate policy and state modules are pure, execution-disconnected, configuration-free, and unreachable from the runtime graph. They exist only for post-closure deployment-readiness work; current percentage-based sizing and all execution guards are unchanged.
