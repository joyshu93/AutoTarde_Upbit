...
The broader higher-timeframe trend, pullback, thesis-failure, ADD-limit, and cooldown-control scenario research is deferred until the ADD loss-attribution diagnostic is complete and verified. It must be designed and evaluated as a separate read-only research phase; no deferred scenario is introduced, selected, or deployed by the current diagnostic.
