...
Before each `LIVE` scheduled cycle, the scheduler must re-run the same persisted-health preflight and block the strategy cycle before any order intent is created if account health has gone stale or unsafe since startup.
