...
Persisted candidate projection fault events are terminal-sweep dispositions only when the same immutable event identity and timestamp have a matching automatic fault-pause transition. Startup reconciliation repairs event-written/pause-missing rows atomically, including recovered-projection faults, while malformed deployment activation evidence remains eligible fault work and cannot escape reconciliation classification.
