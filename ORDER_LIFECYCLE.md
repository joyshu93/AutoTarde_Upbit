...
Uncertain `RECONCILIATION_REQUIRED` and `SUBMITTING` recovery is lookup-only and persists every UUID-first/identifier-fallback observation using an injected clock. A first not-found remains uncertain; only the configured persisted not-found count and persisted elapsed-time bounds together can confirm terminal absence. Transient lookup failures never count as absence. Found recovery records the exchange snapshot but never resumes execution, clears pause/degraded/kill-switch state, or releases an uncertainty-retained lease.

After terminal order and fill evidence is durable, the BTC candidate projector may process an eligible `STRATEGY` order once through its atomic candidate evidence/state-CAS/audit repository contract. It requires a valid persisted PositionGuard decision, matching provenance, terminal `FILLED` or `CANCELED` status, nonzero aggregate fills, exact decimal quantities/values, and a confirmed KRW fee for every fill. Terminal no-fill cancellation is an explicit no-op; invalid, missing, or conflicting evidence records a fault and pauses without any send or execution resumption.
