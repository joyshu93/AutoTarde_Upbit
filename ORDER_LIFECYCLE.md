...
The sweep treats a candidate fault as disposed only when the immutable event has a matching automatic fault-pause transition. On restart, an event-written/pause-missing standard or recovered-projection fault is replayed through the atomic pause operation with its original event timestamp. Malformed deployment activation persistence remains eligible for this fail-closed repair path. Separately, a `FAILED` order already finalized with `ORDER_SUBMISSION_ABSENCE_CONFIRMED` is excluded before reconciliation lookup candidates and budgets are built.
