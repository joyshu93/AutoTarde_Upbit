...
The verified LIVE file identity is reasserted around every later SQLite open using the main-database location reported by that opened connection, and the migration/account/instance/credential contract is validated on the same handle before ownership, migration, or bootstrap mutation. Replacing the database or supplying a different actual handle after initial verification therefore fails before an order lifecycle or runtime-ownership event can be created in the replacement.
...
7. renew and verify lease ownership and perform one bounded account-wide submission-blocking read for every send path. Active lifecycle rows block. `FAILED`/`REJECTED` rows block only when persisted lifecycle, event, exchange-response, or recovery evidence says transmission may have occurred and remains unresolved; definitive pre-send terminal rows, exact persisted exchange rejection, and bounded absence with no `FOUND` evidence are non-blocking. Saturation fails closed. Every path then exactly re-reads its own account-plus-order-ID `SUBMITTING` row; candidate execution additionally re-reads the first event, persisted `READY` decision, deployment, exact candidate state, and binding, projects only status to `PERSISTED`, and reruns the reviewed aggregate validator. The exact own-row read is the final order-state await before synchronous authority validation. `submitOrderFromDecision` then uses one SQLite statement to atomically assert the persisted runtime generation plus account execution status, kill switch, mode, and live gate, and invokes the tagged execution adapter exactly once inside the same narrowly typed non-async authority callback, before the authority method returns its awaitable result and with no intervening helper-resolution, report, database, timer, or promise seam; the `LIVE` adapter requires persisted `LIVE`/`ENABLED` both initially and finally, while the `DRY_RUN` adapter accepts any unchanged non-fully-live tuple, including `DRY_RUN`/`ENABLED` and `LIVE`/`DISABLED`
8. after `createOrder` fulfills or rejects, reassert the same current runtime ownership before any order/fill/rejection persistence, pause, notification, or account-lease release. If that assertion fails, preserve its exact error and leave the durable `SUBMITTING` row plus lease untouched for current-owner recovery. Only a still-current owner may atomically store accepted, definitive rejection, or uncertain-submission evidence; every immediate `FILLED` result includes distinct submitted and terminal event ids plus fill evidence in the same transaction
9. reconcile until terminal state is consistent

`ACCOUNT_EXECUTION_LEASE_MS` is a positive explicit setting with a `30000` default. Every acquisition checks canonical account-wide submission-blocking authority before inserting or taking over a lease, including when no lease row exists. Safe pre-send exits and definitive terminal outcomes release the lease. Uncertain, active, and post-send persistence-failure outcomes retain it. A lease conflict creates no order row or exchange send, persists `ACCOUNT_EXECUTION_LEASE_BLOCKED`, and requires an automatic pause; pause, renewal, acquisition, or release ambiguity fails closed with fatal safety evidence.
...
Uncertain `SUBMITTING`, `RECONCILIATION_REQUIRED`, `FAILED`, and `REJECTED` recovery is lookup-only and persists every UUID-first/identifier-fallback observation using an injected clock. A first not-found remains uncertain; only the configured persisted not-found count and persisted elapsed-time bounds together can atomically compare-and-set terminal absence, its order event, and a fail-closed pause. Any durable canonical `FOUND` observation permanently prevents later not-found observations from confirming absence. Transient lookup failures never count as absence. Before `FOUND`, status, or fills are persisted, every returned snapshot is descriptor-safely copied and exactly matched to the lookup identity and immutable local market, side, order type, canonical decimal price/volume, time-in-force, and SMP material. Upbit market-buy quote amounts preserve decimal intent through eight places and truncate only excess places before decision persistence and transmission; the execution boundary requires that exact persisted decision amount to match the wire `price` and requires null `volume`. Snapshot binding applies the same observed eight-place equivalence to market-buy quote amounts and market-sell volumes so Upbit-normalized responses and legacy binary-float artifacts can recover, without weakening UUID, identifier, market, side, order-type, time-in-force, SMP, or any material difference within those eight decimal places. Found recovery never resumes execution, clears pause/degraded/kill-switch state, or releases an uncertainty-retained lease. A recovered snapshot whose terminal projection fails is explicitly faulted and paused rather than reported as a plain recovery.

When an exactly bound exchange snapshot is successfully applied, the mutable order projection must clear any prior `failure_code` and `failure_message`; those fields describe an active unresolved failure and must not survive a confirmed exchange state. The immutable `order_events`, recovery observations, risk evidence, and raw exchange response continue to preserve the historical failure and recovery trail. This projection cleanup does not resume execution, clear a pause, or release an uncertainty-retained lease.
...
`inspect:btc-pilot:readiness` and its `scripts/inspect-btc-pilot-readiness.example.ps1` wrapper inspect an explicit existing database and explicit account/deployment identity read-only. Readiness requires the append-only candidate binding schema, applies the runtime submission-blocking classification with order IDs/reasons, and proves every terminal evidence row against its exact terminal source order, account/deployment/policy authority, binding material hash, and immutable bound order shape. The checked-in wrapper forces the default `BASELINE` policy selection and contains no candidate selection or confirmation. They do not create, submit, cancel, retry, reconcile, or mutate orders; do not start the app or scheduler; do not call Upbit or Telegram; do not enable LIVE orders; and do not activate or change the pilot. Candidate selection and confirmation require separate local configuration, no-order validation, operator review, and a later explicit activation request; inspection and activation are separate lifecycle events.
...
Candidate intent-fault timestamps are resolved only through the repository's dedicated `pauseForCandidateIntentFault` capability; `ExecutionService` receives that narrow capability instead of the ordinary pause method. The ordinary `pauseForRecoveryFault` input has no timestamp policy and rejects extra keys, so it always preserves strict caller-supplied chronology. Before canonical successor resolution, the dedicated path requires exact `CANDIDATE_INTENT_FAULT_V1` provenance, an allowed `DUPLICATE`/`DERIVATION`/`PERSISTENCE` stage, matching deployment/account/decision/order/binding identities, the correct stage-specific reason, and a deterministic `candidate-intent:<sha256(provenanceJson)>` fault ID. Only then may the serialized repository transaction choose the attempted instant or exactly one nanosecond after the latest deployment/audit chronology. Retries reuse the persisted effective timestamp, while deterministic fault identity and immutable provenance remain based on the failed attempt.
...
Runtime cleanup is a clean `RELEASED` handoff only when all lifecycle-adjacent workers, including a scheduler startup-block notification already in flight, have settled and the process lock remains held. Every such transport is ownership-checked before and after its external await. Process-lock loss during cleanup is retained as `LOST`. A delivery or other worker timeout in either long-running or scoped shutdown also records `LOST` and retains the SQLite/process-lock scope until terminal process exit, so an already-started outbound side effect cannot overlap a newly admitted runtime.
...
The `smoke:dryrun:readiness` script reads local runtime configuration and persisted readiness evidence only; it must not run `/sync`, run strategies, start background workers, deliver notifications, call Upbit, or create order-lifecycle records. It makes no trading or business-state mutation while acquiring and releasing process plus persisted runtime-control ownership evidence, and it contends with an existing owner.
