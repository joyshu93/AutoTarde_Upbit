...
- candidate order submission now re-reads active orders, the exact `SUBMITTING` order, first event, persisted `READY` decision, deployment, exact candidate state, and binding immediately before the final execution-state authority read. Any competitor or authority/material mismatch atomically pauses candidate and global execution, retains the lease and bound order for recovery, and transmits no order.
