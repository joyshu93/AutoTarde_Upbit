...
Upbit market-buy quote amounts preserve decimal intent through eight places and truncate only excess places before the strategy decision, order intent, and outbound request are persisted or sent. Reconciliation uses the same observed eight-place equivalence for `bid`/`price` quote amounts and `ask`/`market` volumes so an Upbit-normalized response can recover a matching local order with excess decimal places, while UUID, identifier, market, side, order type, time-in-force, SMP, and differences within the accepted precision remain fail-closed.
