...
The broader future research scenarios `HTF_TREND_GATE`, `STRICT_PULLBACK`, `EARLY_THESIS_FAILURE`, `ADD_LIMITED`, `COOLDOWN_CONTROL`, and `COMBINED_CONSERVATIVE` remain deferred. They are intentionally outside the current ADD loss-attribution feature and must be introduced only through a separate read-only design, fixed-window evaluation, and review after this diagnostic is complete. This report does not select, simulate, or deploy any of those deferred scenarios.
