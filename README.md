...
Enable the section only by adding `--broad-strategy-hypothesis-profile BROAD_LOSS_CAUSE_V1` to `report:strategy-evaluation` together with the exact frozen matrix. The report preserves per-asset status and provenance and labels cross-asset output non-adjudicative. `ELIGIBLE_FOR_SHADOW_TEST` is a research classification only: it does not modify PositionGuard, start a runtime, call Upbit, submit an order, or approve deployment.
