...
11. Telegram inspection currently includes `/help`, `/config`, `/readiness`, `/status`, `/status detail`, `/statehistory`, `/synchistory`, `/recovery`, `/alerts`, `/risks`, `/balances`, `/positions`, `/orders`, `/order <order-id|identifier>`, `/scheduler`, `/inbound`, and `/sync` for operator visibility. `/status` is the concise locale-aware operator view, while `/status detail` preserves the canonical technical reconciliation, recovery, scheduler, blocker, and transition fields.
