...
11. Telegram inspection currently includes `/help`, `/config`, `/readiness`, `/readiness detail`, `/status`, `/status detail`, `/statehistory`, `/synchistory`, `/recovery`, `/alerts`, `/risks`, `/balances`, `/balances detail`, `/positions`, `/positions detail`, `/orders`, `/order <order-id|identifier>`, `/scheduler`, `/inbound`, and `/sync` for operator visibility. The default portfolio and health views are concise and locale-aware, while their `detail` forms preserve the canonical technical fields.
