...
11. Telegram inspection currently includes `/help`, `/config`, `/readiness`, `/readiness detail`, `/status`, `/status detail`, `/statehistory`, `/synchistory`, `/recovery`, `/alerts`, `/alerts detail`, `/risks`, `/risks detail`, `/balances`, `/balances detail`, `/positions`, `/positions detail`, `/orders`, `/orders detail`, `/order <order-id|identifier> [detail]`, `/scheduler`, `/scheduler detail`, `/inbound`, and `/sync` for operator visibility. The default portfolio, health, order, risk-history, alert-delivery, and scheduler views are concise and locale-aware, while their `detail` forms preserve the canonical technical fields.
