...
- candidate order submission now performs a bounded read of account active and potentially dispatched recovery rows, then re-reads the exact account-plus-order-ID `SUBMITTING` order, first event, persisted `READY` decision, deployment, exact candidate state, and binding. `FAILED` and `REJECTED` uncertainty blocks while absence-confirmed `FAILED` does not; strict descriptor-safe record projections prevent accessor/prototype laundering. The final execution-state await is in the submit method directly before synchronous authority checks and the sole send. Any competitor, saturation, or authority/material mismatch atomically pauses candidate and global execution, retains the lease and bound order for recovery, and transmits no order.
