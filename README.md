...
Validation windows must be explicit, ordered, non-overlapping `[from,to)` ranges. Select boundaries from persisted dataset coverage; a window with a missing expected frame remains visible but is classified as insufficient rather than interpolated. Each asset and cost cell is replayed once from its supplied initial state, and window metrics slice that continuous path without resetting cash, inventory, or prior decisions. The section reports period return, drawdown, turnover, fees and completeness, completed and carry-in episodes, suppressed-ADD exposure, frame coverage without interpolation, and paired `NO_ADD - BASELINE` percentage-point deltas. Its classification is descriptive and explicitly makes no statistical-significance claim.
