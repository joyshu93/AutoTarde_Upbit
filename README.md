...
- uncertain submission recovery is UUID-first/identifier-fallback and lookup-only, with injected-clock persisted observations; first absence remains uncertain, transient errors do not count as absence, and only explicit count-plus-elapsed bounds can fault terminal absence without resending or resuming execution
- eligible terminal BTC strategy orders now project exact-decimal candidate evidence only after durable fill backfill and complete persisted decision/order/fill/fee validation; the atomic candidate evidence/state/audit write is restart-idempotent, while no-fill cancellation is a no-op and invalid evidence pauses safely
