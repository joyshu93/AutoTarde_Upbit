...
- eligible terminal BTC strategy orders now project exact-decimal candidate evidence after durable fill backfill and a separate bounded terminal sweep; immutable deployment bindings and individually confirmed per-fill fees are required, the atomic candidate evidence/state/audit write is restart-idempotent, terminal no-fill cancellations persist replay-safe no-op markers, and a fault is disposed only with its matching automatic-pause transition. Restart sweeps repair event-written/pause-missing standard and recovered-projection faults with the immutable event timestamp, while malformed activation persistence faults and pauses instead of escaping the sweep
