...
import { formatStrategySchedulerPresentation } from "./scheduler.js";
import { formatHelpPresentation } from "./help.js";
import {
...
import { formatOrdersPresentation } from "./orders.js";
import { formatOrderDetailPresentation } from "./order-detail.js";
import { formatRiskEventsPresentation } from "./risks.js";
...
import { formatControlPresentation } from "./control.js";
import { formatSyncPresentation } from "./sync.js";
import { formatStrategyPreviewPresentation } from "./preview.js";
import { formatStrategyRunPresentation } from "./run.js";
import { formatRuntimeConfigPresentation } from "./config.js";
import {
  formatReconciliationHistoryPresentation,
  formatStateHistoryPresentation,
...
export function formatRuntimeConfigMessage(
  config: TelegramRuntimeConfigSnapshot | null,
  locale: TelegramLocale = DEFAULT_TELEGRAM_LOCALE,
): string {
  return formatRuntimeConfigPresentation(
    config,
    formatRuntimeConfigTechnicalMessage(config),
    locale,
  );
}

function formatRuntimeConfigTechnicalMessage(config: TelegramRuntimeConfigSnapshot | null): string {
  if (!config) {
    return [
      "Runtime Config",
...
export function formatStateHistoryMessage(
  ...
  return formatStateHistoryPresentation(
    transitions,
    formatStateHistoryTechnicalMessage(transitions),
    locale,
  );
}

function formatStateHistoryTechnicalMessage(
  transitions: ExecutionStateTransitionRecord[],
): string {
...
export function formatReconciliationRunsMessage(
  ...
  const summaries = runs.map((run) => {
    ...
    return {
      ...
      issueCodes: meta.issueCodes,
      historyRecovery: toHistoryRecoveryPresentationSummary(meta.historyRecovery),
    };
  });

  return formatReconciliationHistoryPresentation(
    summaries,
    formatReconciliationRunsTechnicalMessage(runs),
    locale,
  );
}

function formatReconciliationRunsTechnicalMessage(
  runs: ReconciliationRunRecord[],
): string {
  if (runs.length === 0) {
    return [
...
export function formatRecoveryProgressMessage(
  ...
  return formatRecoveryPresentation(
    latestRun,
    toHistoryRecoveryPresentationSummary(meta?.historyRecovery ?? null),
    checkpoints,
    technicalBody,
    locale,
  );
...
export function formatStrategyRunMessage(
  result: TelegramStrategyRunResult,
  locale: TelegramLocale = DEFAULT_TELEGRAM_LOCALE,
): string {
  return formatStrategyRunPresentation(result, locale);
}
...
function toHistoryRecoveryPresentationSummary(
  historyRecovery: ReturnType<typeof tryParseReconciliationSummaryMeta>["historyRecovery"],
): HistoryRecoveryPresentationSummary | null {
  if (!historyRecovery) {
    return null;
  }

  return {
    coverageStatus: historyRecovery.coverageStatus,
    confidenceLevel: historyRecovery.confidenceLevel,
    confidenceReason: historyRecovery.confidenceReason,
    retentionStatus: historyRecovery.retentionStatus,
    failureMessage: historyRecovery.failureMessage,
    recoveredOrderCount: historyRecovery.recoveredOrderCount,
    scannedSnapshotCount: historyRecovery.scannedSnapshotCount,
    markets: historyRecovery.markets.map((market) => ({
      market: market.market,
      archiveComplete: market.archiveComplete,
      nextWindowEndAt: market.nextWindowEndAt,
    })),
  };
}
